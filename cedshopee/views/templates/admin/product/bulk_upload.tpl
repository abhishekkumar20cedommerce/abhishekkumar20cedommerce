...
<div class="panel">
    <h3><i class="icon-tag"></i> {l s='Shopee Bulk Product Upload' mod='cedshopee'}</h3>

    <div class="row">
        <div class="buttons"><button id="uploadAllAction" data-token="{$token|escape:'htmlall':'UTF-8'}" class="btn btn-primary" onclick="processReport();">Process Product Upload</button></div>
    </div>
    <div class="row">
...
<script type="text/javascript">
    ...
    function sendUpdateRequest(chunked_array,url){
        ...
        if(chunked_array[len]){
            $.ajax({
                type: "POST",
                url: 'index.php?controller=AdminCedShopeeBulkUploadProduct&method=ajaxProcessBulkUpload&token={$token|escape:'htmlall':'UTF-8'}',
                data: {
                    ajax: true,
                    controller: 'AdminCedShopeeBulkUploadProduct',
                    action: 'bulkUpload',
